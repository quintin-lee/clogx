...
static double g_fill_rate = 0.0; /* tokens per millisecond */
...
void log_rate_limit_init(bool enable, int max_per_sec, int burst) {
    ...
    if (enable && max_per_sec > 0 && burst > 0) {
        ...
        g_fill_rate = (double)max_per_sec / 1000.0;
        g_last_update_ms = get_now_ms();
...
bool log_rate_limit_allow(uint64_t *out_suppressed_count) {
    ...
    if (now_ms > g_last_update_ms) {
        double elapsed_ms = (double)(now_ms - g_last_update_ms);
        g_tokens += elapsed_ms * g_fill_rate;
