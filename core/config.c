                ...
                if (in_plugin_config) {
                    if (expect_key) {
                        ...
                        if (rem > 0 && plugin_json_len > 0) {
                            int written = snprintf(plugin_json + plugin_json_len,
                                                   (size_t)rem,
                                                   "%s\"%s\":\"",
                                                   plugin_json_len > 1 ? "," : "",
                                                   plugin_cfg_key);
                ...
                if (found) {
                    ...
                    case HANDLER_QUEUE_SIZE: {
                        ...
                        if (end == val || *end != '\0' || errno == ERANGE || qs <= 0) {
                            fprintf(stderr,
                                    "Invalid queue_size: %s (must be a positive integer)\n",
                                    val);
                            has_errors = 1;
                        } else {
                    ...
                    case HANDLER_SOCKET_TLS_CA_FILE:
                        snprintf(
                            cfg->socket_tls_ca_file, sizeof(cfg->socket_tls_ca_file), "%s", val);
                        break;
                    ...
                    case HANDLER_PROMETHEUS_PORT: {
                        ...
                        if (end == val || *end != '\0' || errno == ERANGE || p <= 0 || p > 65535) {
                            fprintf(
                                stderr, "Invalid prometheus_port: %s (must be 1..65535)\n", val);
                            has_errors = 1;
                        } else {
...
static int apply_config(const log_config_t *cfg)
{
    ...
    snprintf(g_default_logger.config.file_path,
             sizeof(g_default_logger.config.file_path),
             "%s",
             cfg->file_path);
    snprintf(g_default_logger.config.socket_host,
             sizeof(g_default_logger.config.socket_host),
             "%s",
             cfg->socket_host);
    ...
    for (int i = 0; i < cfg->plugin_count && i < CLOG_MAX_PLUGINS; i++) {
        snprintf(g_default_logger.config.plugin_so_paths[i],
                 sizeof(g_default_logger.config.plugin_so_paths[0]),
                 "%s",
                 cfg->plugin_so_paths[i]);
        snprintf(g_default_logger.config.plugin_params_json[i],
                 sizeof(g_default_logger.config.plugin_params_json[0]),
                 "%s",
    ...
    if (cfg->format) {
        snprintf(
            g_default_logger.format_str, sizeof(g_default_logger.format_str), "%s", cfg->format);
        g_default_logger.config.format = g_default_logger.format_str;
    }
    if (cfg->time_format) {
        snprintf(g_default_logger.time_format_str,
                 sizeof(g_default_logger.time_format_str),
                 "%s",
                 cfg->time_format);
